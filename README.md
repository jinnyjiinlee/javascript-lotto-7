...
- [x] 사용자가 구매한 수대로 생성된 로또가 저장이 된다. 

    출력

- [x] 구매한 로또 리스트를 한 줄씩 출력해 준다.

### 당첨 번호

입력

- [X] 사용자에게 당첨번호 숫자 6개를 를 받는다.
...
예외 처리
- [X] 당첨 번호 각각은 로또의 범위인 0초과 46미만으로 일치해야 한다. 
- [X] 중복된 번호는 입력할 수 없다. 
- [X] 사용자가 잘못된 값을 입력할 경우 `throw`문을 사용해 예외를 발생시킨다. 그런 다음, "[ERROR]"로 시작하는 에러 메시지를 출력하고 해당 부분부터 입력을 다시 받는다.
...
- [ ] 당첨번호와 중복된 번호는 입력받을 수 없다. 
- [X] 사용자가 잘못된 값을 입력할 경우 `throw`문을 사용해 예외를 발생시킨다. 그런 다음, "[ERROR]"로 시작하는 에러 메시지를 출력하고 해당 부분부터 입력을 다시 받는다.
- [X] e.g. [ERROR] 로또 번호는 1부터 45 사이의 숫자여야 합니다.
